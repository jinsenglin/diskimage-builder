...
fn_array=(show-env build-cloud_init_dev build-license_dev build-vm_c7 build-vm_c7_sc_dev build-vm_c7_sc_1 build-vm_c7_sc_2 build-bm_c7 build-bm_c7_k80 build-bm_c7_k80_nvidia_docker)
...
function build_vm_c7_sc_1() {
    ...
    export DIB_SC_DASHBOARD=http://localhost:8000/cascade-dashboard-1.1.6-rc.105-saas.tgz
    export DIB_SC_DASHBOARD_RUN_AFTER_INIT=false
    export DIB_SC_DASHBOARD_PROTECTED=false
...
function build_vm_c7_sc_2() {
    ...
    if [ ${ans:-y} == "y" ]; then
        git checkout sc -- elements/license/install.d/11-license
        #disk-image-create -t raw centos7 vm dhcp-all-interfaces selinux-permissive devuser cloud-init-nocloud cloud-init-patch license ansible sc-dashboard -o vm-c7-sc-dev --image-size 4
        disk-image-create -t raw centos7 vm dhcp-all-interfaces selinux-permissive devuser cloud-init-patch license ansible sc-dashboard -o vm-c7-sc-2 --image-size 4
    fi
}
